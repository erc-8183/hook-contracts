 ...
 *         of job completion and rejection using ERC-8001 standard.
 *
 * USE CASE
 * --------
 ...
 * multi-party consensus through EIP-712 attestations.
 *
 * FLOW
 * ----
 ...
 *     → Delegates to ERC-8001 contract
 ...
 *     → If not Ready, reverts with custom error
 ...
 * KEY PROPERTIES
 * --------------
 * - Standard-compliant ERC-8001 coordination
 ...
 * - Gas-efficient: minimal state in hook, complex logic in ERC-8001
 */
contract ERC8001CoordinationHook is BaseACPHook {
    ...
    enum ActionType { None, Complete, Reject }
    ...
    constructor(address acpContract_, address coordinationContract_) 
    ...
    function proposeCoordination(
        ...
        modifiedIntent.coordinationType = actionType == ActionType.Complete 
            ? COORDINATION_COMPLETE 
            : COORDINATION_REJECT;
        
        // Call ERC-8001 to propose
        intentHash = coordinationContract.proposeCoordination(
            modifiedIntent,
            signature,
            payload
        );
        ...
        jobCoordinations[jobId] = CoordinationInfo({
    ...
    function acceptCoordination(
        uint256 jobId,
        IERC8001.AcceptanceAttestation calldata attestation
    ...
    function executeCoordination(
        ...
        (success, result) = coordinationContract.executeCoordination(
            info.intentHash,
            payload,
            executionData
        );
