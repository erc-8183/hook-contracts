...
| [WalletStateHook.sol](./contracts/hooks/WalletStateHook.sol) | A — Simple Policy | Condition-based wallet-state gating for `_preFund`. Verifies the funding wallet against a named condition set (e.g. stablecoin threshold, NFT held, KYC attested) before the job escrow can form. Uses an external `IWalletStateVerifier` so attestation sources stay pluggable; reference implementation at [`contracts/examples/InsumerWalletStateVerifier.sol`](./contracts/examples/InsumerWalletStateVerifier.sol). |
